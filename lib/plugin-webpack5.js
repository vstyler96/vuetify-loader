  ...
  apply(compiler) {
    ...
    for (const rawRule of rules) {
      ...
      if (vueRules.length > 0) {
        if (rawRule.oneOf) {
          throw new Error(
              `[VuetifyLoaderPlugin Error] vue-loader 15 currently does not support vue rules with oneOf.`
          )
        }
        break
      }
    ...
    const vueUse = rules.filter(rule => rule.use && rule.use.filter(isVueLoader))
    vueUse.forEach(this.updateRule.bind(this))
